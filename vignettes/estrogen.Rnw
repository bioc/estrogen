...
%\VignetteDepends{Biobase, affy, vsn, genefilter, hgu95av2.db}
...
jpeg("estrogen-image1.jpg", quality = 100)
image(a[, 1])
dev.off()
@
%
...
image(badc)
@
<<images2b,eval=FALSE,echo=FALSE,results=hide>>=
jpeg("estrogen-image2.jpg", quality = 100)
badc <- ReadAffy("bad.cel")
image(badc)
dev.off()
@
...
%
<<scp-plots,eval=FALSE,echo=FALSE,results=hide>>=
jpeg("estrogen-scp1.jpg", quality=100)
plot(exprs(a)[, 1:2], log="xy", pch=".", main="all")
dev.off()

jpeg("estrogen-scp2.jpg", quality=100)
plot(pm(a)[, 1:2], log="xy", pch=".", main="pm")
dev.off()

jpeg("estrogen-scp3.jpg", quality=100)
plot(mm(a)[, 1:2], log="xy", pch=".", main="mm")
dev.off()

jpeg("estrogen-scp4.jpg", quality=100)
...
@
<<heatmapc,results=hide,echo=FALSE,eval=FALSE>>=
jpeg("estrogen-heatmap.jpg", quality = 100)
heatmap(exprs(x)[sel,], col=gentlecol(256))
dev.off()
@
